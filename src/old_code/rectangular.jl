...
function setup_domain(L₁,h,partition,B,D,pmid)
    pmin = Point(0.0, -h)
    pmax = Point(L₁, 0.0)
    geo = quadrilateral(;x0=pmid,d1=VectorValue(B/2,0.0),d2=VectorValue(0.0,D))
    # model = CartesianDiscreteModel(pmin, pmax, partition)
    model = GmshDiscreteModel("data/meshes/background5.msh")
...
@timeit to "domain" begin
    ...
    L₁ = 2*λ
    for HR in HR_ratios
        model, geo = setup_domain(L₁,h,(n₁,n₂),B,D,Point(0.0,-D))
        push!(models, model)
        push!(geos, geo)
    end # for
end
