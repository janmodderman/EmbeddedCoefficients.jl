...
function setup_domain(parts,B,D,pmid)
    geo = quadrilateral(;x0=pmid,d1=VectorValue(B/2,0.0),d2=VectorValue(0.0,D))
    # model = CartesianDiscreteModel(pmin, pmax, partition)
    model = GmshDiscreteModel(parts,"data/meshes/background.msh")
...
function setup_spaces(order::Int64, model, Ω, cutgeo)
...
function setup_spaces(order::Int64, model, Ω)
    reffeᵩ = ReferenceFE(lagrangian, Float64, order)
...
function setup_interiors(model,cutgeo,cutgeo_facets,degree)
    ...
    facets = BoundaryTriangulation(cutgeo_facets,tags=["surface"])
    Γf⁻ = BoundaryTriangulation(cutgeo)
...
function main_agfem(KR, model, cutgeo, cutgeo_facets)
    order = 1
    degree = 2 * order
    D=0.1
    g=9.81
...
function main_cutfem(KR, model, cutgeo, cutgeo_facets)
    order = 1
    degree = 2 * order
    D=0.1
    g=9.81
...
function main_sbm(KR, model, cutgeo, cutgeo_facets)
    order = 1
    degree = 2 * order
    D=0.1
    g=9.81
...
function main2D(nparts,distribute)
    ...
    HR_ratios = [0.0]
    for HR in HR_ratios
        model, geo = setup_domain(parts,B,D,Point(0.0,-D))
        push!(models, model)
        push!(geos, geo)
    end # for
    for (i, model) in enumerate(models)
        ...
        for KR in KRs
            Mₐ, Cₐ = main_sbm(KR, model,cutgeo, cutgeo_facets)
